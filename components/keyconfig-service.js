...
keyconfigService.prototype = {
  ...
  init: function(event) {
    ...
    var XUL = (typeof this.document.createXULElement === "function");

    if (event && event.eventPhase != 2) return;

    this.removeEventListener("pageshow", this.keyconfig.service.init, false);

    this.keyconfig.removedKeys =
      this.document.documentElement.appendChild(XUL ? this.document.createXULElement("keyconfig")
                                                    : this.document.createElement("keyconfig"));

    if (this.keyconfig.service.ps.prefHasUserValue("keyconfig.global.20110522")) {
      // Convert old preferences to new ones
      var oldBranch = "keyconfig.";
      var newBranch = "extensions.dorandoKeyConfig.";
    ...
    var profile;
    try {
      profile = this.keyconfig.service.ps.getCharPref("extensions.dorandoKeyConfig.profile");
    } catch(ex) {
      profile = "main";
    }
    this.keyconfig.profile = "extensions.dorandoKeyConfig." + profile + ".";

    var i, l;

    var keyset = this.document.getElementsByTagName("keyset")[0] ||
                 this.document.documentElement.appendChild(XUL ? this.document.createXULElement("keyset")
                                                               : this.document.createElement("keyset"));
    ...
    for (i = 0, l = keys.length; i < l; i++) {
      ...
      if (key[3] && (!key[4] || key[4] == this.document.location)) {
        node = keyset.appendChild(XUL ? this.document.createXULElement("key")
      ...
      if (key[0] == "!") {
        this.keyconfig.removedKeys.appendChild(node);
        continue;
      }
