...
var gPrefBranch = gPrefService.getBranch("");

var gDefaultBranch = gPrefService.getDefaultBranch("extensions.dorandoKeyConfig.");
gDefaultBranch.setBoolPref("devmode", false);
gDefaultBranch.setBoolPref("devmode", false);
gDefaultBranch.setBoolPref("nicenames.reverse_order", false);
gDefaultBranch.setCharPref("profile", "main");
gDefaultBranch.setBoolPref("warnOnDuplicate", true);
gDefaultBranch.setBoolPref("allowAltCodes", true);
gDefaultBranch.setBoolPref("hideDisabled", false);
...
function onLoad() {
  ...
  switch (gPrefBranch.getIntPref("ui.key.accelKey")) {
  ...
  gReverseNames = isThunderbird ^ gPrefBranch.getBoolPref("extensions.dorandoKeyConfig.nicenames.reverse_order");

  if (gPrefBranch.getBoolPref("extensions.dorandoKeyConfig.devmode")) { this.getFormattedKey = function(a, b, c) {return (a+"+"+b+c).replace(/null/g, "");} }
  ...
  for (i = 0, l = windowList.firstChild.childNodes.length; i < l; i++) {
    if (windowList.firstChild.childNodes[i].hasAttribute("label") && windowList.firstChild.childNodes[i].getAttribute("label") == target.document.title) {
...
function init(target) {
  ...
  var hideDisabled = gPrefBranch.getBoolPref("extensions.dorandoKeyConfig.hideDisabled");
...
function Recognize(event) {
  ...
  if (!(gPrefBranch.getBoolPref("extensions.dorandoKeyConfig.allowAltCodes") && modifiers == "alt" && key && !isNaN(key))) {
    if (gPrefBranch.getBoolPref("extensions.dorandoKeyConfig.warnOnDuplicate") && gEdit.value != gEdit.key.shortcut && gUsedKeys[gEdit.value]) {
...
function Apply() {
  ...
  try {
    // Gecko 58+
    gPrefBranch.setStringPref(gProfile+key.id, value);
  } catch (ex) {
    var str = Components.classes["@mozilla.org/supports-string;1"].createInstance(Components.interfaces.nsISupportsString);
    str.data = value;
    gPrefBranch.setComplexValue(gProfile+key.id, Components.interfaces.nsISupportsString, str);
  }
...
function Reset() {
  ...
  try {
    gPrefBranch.clearUserPref(gProfile+key.id);
  } catch(ex) {}
...
function Key(aKey) {
  ...
  this.shortcut = getFormattedKey(
    aKey.hasAttribute("modifiers") ? aKey.getAttribute("modifiers")
                                   : null,
    aKey.hasAttribute("keytext") ? aKey.getAttribute("keytext")
                                 : aKey.hasAttribute("key") ? aKey.getAttribute("key").toUpperCase()
                                                            : aKey.hasAttribute("charcode") ? aKey.getAttribute("charcode").toUpperCase()
                                                                                            : null,
    aKey.hasAttribute("keycode") ? aKey.getAttribute("keycode")
                                 : null
  );
  this.id = aKey.id;
  if (aKey.getAttribute("keyconfig") == "resetted") {
    this.shortcut = gStrings.uponreset;
  }

  try {
    try {
      // Gecko 58+
      this.pref = gPrefBranch.getStringPref(gProfile+aKey.id).split("][");
    } catch (ex) {
      this.pref = gPrefBranch.getComplexValue(gProfile+aKey.id, Components.interfaces.nsISupportsString).data.split("][");
    }
  } catch(ex) {
    this.pref = [];
  }
...
Key.prototype = {
  get disabled() {
    return this.shortcut == gStrings.disabled;
  }
}
...
function closeEditor(fields) {
  ...
  if (key.name != fields.name.value) {
    try {
      ...
      } while (gPrefBranch.prefHasUserValue(gProfile+key.id));
    } catch(ex) {
      gUnicodeConverter.charset = "UTF-8";
      key.id = "key"+Date.now();
    }
  ...
  while ((target = targets.getNext())) {
    ...
    if (!node) {
      // createElement was renamed to createXULElement in Gecko 68?
      var XUL = (typeof target.document.createXULElement === "function");

      node = target.keyconfig.removedKeys.appendChild(XUL ? target.document.createXULElement("key")
                                                          : target.document.createElement("key"));
    }
...
var keyView = {
  ...
  getCellProperties: function(row, col) {
    var key = gKeys[row];
    // try { console.log ( key + " " + key.shortcut ) } catch(ex) {};
    //try { if (key.shortcut ) return "reset"; } catch(ex) {console.log(ex)}
